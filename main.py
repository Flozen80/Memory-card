...
question=[
Question('Яблуко','apple', 'watermalon','melon','orange'),
Question('gwg','apple', 'watermalon','melon','orange'),
Question('fhsjn','apple', 'watermalon','melon','orange'),
Question('djh','apple', 'watermalon','melon','orange')
]
...
def click_ok():
    if btn_done.text() == "Віповісти":
        AnsGroupBox.show()
        RadioBoxGroup.hide()
        btn_done.setText("Наступне питання")
        check()
    else:
        btn_done.setText("Віповісти")
        AnsGroupBox.hide()
        RadioBoxGroup.show()
        new_question()
...
def menu_generation():
    ...
    text = (f'Разів відповіли: {Question.count_ans}\n' \
            f'Вірних відповідей: {Question.count_ans}n' \
            f'Успішність: {round(c, 2)}%')
    lb_stat_change.setText(text)
    win_card.hide()
    win_menu.show()

btn_done.clicked.connect(menu_generation)

def back():
    win_card.show()
    win_menu.hide()

btn_done.clicked.connect(back)
...
btn_done.clicked.connect(clear)
